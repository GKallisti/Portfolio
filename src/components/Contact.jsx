...
const Contact = () => {
  ...
  useEffect(() => {
    setIsFormComplete(isFormValid() && validateEmail(form.email));
  }, [form]);
  ...
  return (
    <div className='xl:mt-12 xl:flex-row flex-col-reverse flex gap-10 overflow-hidden  z-[1] '>
      <motion.div variants={slideIn("left", "tween", 0.5, 1.05)} className='flex-[0.75] bg-primary p-8 rounded-1xl '>
        ...
        <form className="mt-12 flex flex-col gap-6" ref={formRef} onSubmit={handleSubmit}>
          ...
          <button type='submit' 
            className={`bg-secondary text-black py-4 px-5 outline-none rounded-1xl font-bold text-[22px] ${isFormComplete ? '' : 'opacity-50 cursor-not-allowed'}`}
      ...
      <motion.div variants={slideIn("right", "tween", 0.5, 1.05)} className='xl:flex-1 xl:h-auto xl:mt-0 md:h-[550px] h-[350px] '>
        <EarthCanvas />
      </motion.div>
    </div>
  );
